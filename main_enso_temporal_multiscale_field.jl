...
#
# Modes:
#   :no_xscale_field    — three independent spatial pipelines, sum predictions.
#                         Tests whether per-pixel band decomposition alone
#                         improves spatial fidelity over single-pipeline E5/E1.
#   :full_cascade_field — temporal cross-scale wiring slow → mid → fast at
#                         the same spatial blocks. Tests whether the slow
#                         band's spatial prediction (richer than 1D scalar
#                         in Stage A) makes cross-scale wiring useful here.
#
...
mode     = Symbol(get(ENV, "ENSO_TM_MODE", "no_xscale_field"))
@assert mode in (:no_xscale_field, :full_cascade_field) (
    "Unknown ENSO_TM_MODE=$(mode); expected no_xscale_field or full_cascade_field")
...
# Variance fraction reporting (sanity for Stage 0 spectra)
total_var = mean(var(reshape(fine_2d, nlon_f * nlat_f, Ttot); dims = 2))
...
# For :no_xscale_field — single-layer 2D blocks per band.
...
# For :full_cascade_field — multi-layer pair where the fine block points to the
# *same-spatial-location* coarse block (same grid, divisor = 1).
blocks_pair = make_blocks([fine_2d, fine_2d], [grid_2d, grid_2d], mixing)
blocks_band_coarse = blocks_pair[1]
blocks_band_fine   = blocks_pair[2]
_, _, layer_dim = input_dimensions(blocks_band_fine)
println("  cross-scale layer dim per fine block: $(layer_dim)  (= rec_dim of coarse)")

# Placeholder for run_single_layer's `data_layer` arg
zero_layer = zeros(Float64, size(slow_mat))
...
# All preds_band_mat are shape (n_pixels, warmup + predict_len). Drop warmup.
...
# Per-band truth/pred for diagnostics
...
jldsave(joinpath(outdir, "enso_temporal_field_preds_$(mode_tag)_$(seed_tag).jld2"); compress = true,
        n34_pred       = Float32.(n34_pred),
        field_pred     = Float32.(field_pred_3d),
        slow_pred      = Float32.(slow_pred_3d),
        mid_pred       = Float32.(mid_pred_3d),
