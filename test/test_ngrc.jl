@testset "NG-RC" begin
    ...
    # combs_with_repetition_mat (internal): combinations are stored by column.
    c = CrossScaleRC.combs_with_repetition_mat(2, 2)
    @test size(c) == (2, 3)
    @test c == [1 1 2; 1 2 2]  # (1,1), (1,2), (2,2)
    @test CrossScaleRC.combs_with_repetition_mat(3, 1) == [1 2 3]
    ...
    past = 2
    degree = 2
    poly_combos = [CrossScaleRC.combs_with_repetition_mat(nlin, p) for p in 2:degree]
    F = CrossScaleRC.feature_length(nlin, degree)
    ϕ = Vector{Float64}(undef, F)
    xhist = randn(2, past)
    CrossScaleRC.fill_features_full!(ϕ, xhist, poly_combos)
    @test ϕ[1] == 1.0
    @test length(ϕ) == F
    @test all(isfinite, ϕ)
    ...
    preds_test, preds_train, Wout = nextgen_closedloop(
        data,
        train_len,
        predict_len;
        washout = washout,
    ...
    preds_test3, _, _ = nextgen_closedloop(data, 50, 15; washout = 2, past = 2, degree = 3, ridge = 1e-4)
